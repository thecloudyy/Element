...
/// <c>HttpServerService.DownloadAndInstallAsync</c>. The ZIP byte sniff and the staged-file cleanup
    ...
    AppliedFixIndexService fixIndex)
{
        ...
        Action? onReveal = null)
    {
        ...
        return new DownloadJob(
            ...
            covers.GetLocalPath(appId),
            (_, progress, ct) => throw new DownloadAbortedException("DLC generation is no longer supported"),
            (file, _, _) => Task.FromResult(InstallManifest(file, appId, title)),
            ConfirmAsync: null,
