...
/// regions) and cached under %AppData%\ElementGui\cloudredirect. It self-updates, so we always grab the
...
public class CloudRedirectService(GithubProxy gh)
{
    private static readonly string ToolDir =
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "ElementGui", "cloudredirect");
