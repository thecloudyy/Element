...
public static partial class LuaFileParser
{
    ...
    // Strips a trailing "(123456) ..." / "デポ" tail that some sources append to depot comments.
    [GeneratedRegex(@"\s*\(\d+\)\s*\S*\s*$")]
    private static partial Regex CommentTailRegex();

    /// <summary>Tidy a raw lua trailing comment into a display name (drops trailing "(id) デポ" tail).</summary>
