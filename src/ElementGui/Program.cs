...
public static class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        // Only one instance may run: two copies share %AppData%\ElementGui (auth.dat,
        // caches) and would lock each other's files (e.g. a token write failing on login).
        using var single = new Mutex(initiallyOwned: true, "ElementGui.SingleInstance", out bool isFirst);
        if (!isFirst)
        {
    ...
    private const string ShowWindowEventName = "ElementGui.ShowWindow";
    ...
    private const string EnableTrayLockEventName = "ElementGui.EnableTrayLock";
    ...
    private const string RecheckUpdatesEventName = "ElementGui.RecheckUpdates";
    ...
    /// <summary>Read just the "Language" key from %AppData%\ElementGui\settings.json without spinning up
